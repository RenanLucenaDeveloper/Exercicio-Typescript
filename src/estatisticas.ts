...
type VendaComValor = Venda & {valor : number}
function filtrarValor(venda: Venda): venda is VendaComValor {
  return venda.valor !== null;
}

function calcularDados(json: Venda[]) {
  const estatisticas : estatisticas = {
    ...
    // Calcula o total e coloca no objeto
    const totalArray = json.filter(filtrarValor).forEach(venda => estatisticas.total += venda.valor);

    json.forEach((item) => {
      // Calcula a quantidade de compras em cada
      // modalidade e coloca no objeto
      ...
      // Define o melhor dia de compras
      // da semana e adiciona no objeto
...
function AdicionarEstatisticas(estatisticas : estatisticas) {
